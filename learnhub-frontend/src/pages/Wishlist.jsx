...
const Wishlist = () => {
  ...
  const fetchFavorites = async () => {
    try {
      ...
      // Fetch full course details for each favorite
      const transformedFavorites = await Promise.all(
        favoritesData.map(async (fav) => {
          ...
          try {
            ...
            // Return basic course info if detail fetch fails
            return {
              courseid: courseId,
              id: courseId,
  ...
  const handleTabClick = (tab) => {
    setActiveTab(tab);
    if (tab === 'all-courses' || tab === 'courses' || tab === 'completed') {
      navigate(`/my-courses?tab=${tab}`);
    }
    // If wishlist tab is clicked, stay on current page
  };
